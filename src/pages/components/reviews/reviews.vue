  ...
  <div class="reviews">
    <div class="reviews__leftPart">
      ...
      <div class="reviews__nav">
        <reviewsPoints :activeElem="activeElem" :index="item.index" :key="item.id" v-for="item of reviews"></reviewsPoints>
      </div>
    </div>
    <div class="reviews__posts">
      <postsMessage :message="message" :key="message.id" v-for="message of reviews"></postsMessage>
    </div>
    <div class="reviews__arrows">
      <span class="reviews__up" @click="isTarget($event.target, 'arrow')"></span>
      <span class="reviews__down __active" @click="isTarget($event.target, 'arrow')"></span>
    </div>
  </div>
</template>
...
export default {
  ...
  data: function () {
    return {
      ...
      points: document.getElementsByTagName('reviewsPoints'),
      activeElem: '',
    }
  },
  methods: {
    isTarget: function (target, on) {
      if (on === 'arrow' && target.className === 'reviews__up' || target.className === 'reviews__down') {
        if (target.className === 'reviews__up' && target.className === 'reviews__up __active') {
          target.classList.remove('__active');

          // следующий сосед __active
          target.nextSibling.classList.add('__active');
        }
        if (target.className === 'reviews__down' && target.className === 'reviews__down __active') {
          target.classList.remove('__active');

          // предыдущий сосед __active
          target.prevSibling.classList.add('__active');
        }
      } else if (on === false) {
        target.classList.remove('__active');
      }
    ...
    resetSelect: function () {
      let node = document.querySelector('.reviews__nav').childNodes;
      for (let i = 0; i < node; i++) {
        node[i].classList.remove('__active');
      }
    }
  },
  computed: {},
